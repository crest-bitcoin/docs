...
sequenceDiagram
    ...
    MarketMaker->>Settlement: 100,000 USD
    Settlement->>User: 99,700 USD (minus 0.3% fee)
    Settlement->>Settlement: Store 300 USD fee
```
...
4. Contract transfer USD from market maker to user
5. User receives 99,700 USD (after 0.3% fee)
...
sequenceDiagram
    ...
    Relayer->>Settlement: settleRFQM() (pays gas)
    Settlement->>USD: transferFrom(user, mm, 100,000 USD)
...
**Execution Flow:**
1. User signs quote for 100,000 USD trade
2. Relayer executes settlement (pays gas)
3. USD transferred to market maker
