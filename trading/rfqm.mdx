...
sequenceDiagram
    ...
    Note over User,USDC: Scenario 1: WCBTC → USDC
    User->>Settlement: Approve WCBTC spending
    User->>Relayer: Sign quote for 1 WCBTC ($100,000)
    Relayer->>Settlement: settleRFQM() (pays gas)
    Settlement->>WCBTC: transferFrom(user, mm, 1 WCBTC)
...
**Execution Flow:**
1. User signs quote for 1 WCBTC trade
2. Relayer submits transaction and pays gas
3. WCBTC transferred directly to market maker
4. Market maker sends USDC to Settlement
5. User receives 99,700 USDC (after 0.3% fee)

### Scenario 2: USDC → Native cBTC
...
sequenceDiagram
    ...
    User->>Relayer: Sign quote for 100,000 USDC
    Relayer->>Settlement: settleRFQM() (pays gas)
...
**Execution Flow:**
1. User signs quote for 100,000 USDC trade
2. Relayer executes settlement (pays gas)
3. USDC transferred to market maker
4. Market maker sends WCBTC to Settlement
5. Settlement unwraps WCBTC and sends native cBTC to user

### Scenario 3: USDC → WCBTC
...
sequenceDiagram
    ...
    USDC->>MarketMaker: 100,000 USDC
    MarketMaker->>Settlement: 1 WCBTC
    Settlement->>User: 0.997 WCBTC ($99,700)
    Settlement->>Settlement: Store 0.003 WCBTC ($300) fee
```
